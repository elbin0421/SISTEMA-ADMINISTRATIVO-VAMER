...
class PlanillaEspecialModel {
    ...
    public static function calcularEmpleado(array $emp, string $tipo, int $anio_pago): array {
        ...
        $ingreso         = !empty($emp['fecha_ingreso']) ? new DateTime($emp['fecha_ingreso']) : clone $periodo_ini;
        $inicio_efectivo = $ingreso > $periodo_ini ? $ingreso : clone $periodo_ini;
        $total_dias      = (int)$periodo_ini->diff($periodo_fin)->days + 1;
        $dias_trabajados = max(0, min((int)$inicio_efectivo->diff($periodo_fin)->days + 1, $total_dias));
    ...
    public static function previsualizar(string $tipo, int $anio_pago, array $excluidos = [], int $empresa_id = 0): array {
